...
outputs=list(list(0 for x in range(37))for x in range(45))
x=0
y=0
...
while True:
    ...
    if codes[i]=="99":
        ...
        if int(codes[a])==10:
            ...
            #outputs[y][x%37]=chr(int(codes[a]))
            x+=1
        print(chr(int(codes[a])), end='')
        i+=2
...
for i in range(1,44):
    for j in range(1,36):
        if outputs[i][j]=="#" and outputs[i+1][j]=="#" and outputs[i-1][j]=="#" and outputs[i][j+1]=="#" and outputs[i][j-1]=="#":
            total+=i*j
#             print(i,j)
for x in range(45):
    print(*outputs[x])
